SUBROUTINE DELNOD(K, N, X, Y, Z, LIST, LPTR, LEND, LNEW, LWK, IWK, IER)
    INTEGER K, N, LIST(*), LPTR(*), LEND(*), LNEW, LWK, IWK(2, *), IER
    REAL X(*), Y(*), Z(*)
!
!***********************************************************
...
!
    ...
    INTEGER I, IERR, IWL, J, LNW, LP, LP21, LPF, LPH, LPL, LPL2, LPN, LWKL, N1, N2, NFRST, NIT, NL, NN, NNB, NR
    LOGICAL LEFT
    LOGICAL BDRY
...
!
    N1 = K
    NN = N
    IF (N1 .LT. 1 .OR. N1 .GT. NN .OR. NN .LT. 4 .OR. LWK .LT. 0) GO TO 21
    LPL = LEND(N1)
    LPF = LPTR(LPL)
    NNB = NBCNT(LPL, LPTR)
    BDRY = LIST(LPL) .LT. 0
...
!
    LPL2 = LEND(N2)
    IF (.NOT. ((BDRY .OR. LEFT(XR, YR, ZR, XL, YL, ZL, X1, Y1, Z1)) &
               .AND. (LIST(LPL2) .LT. 0 .OR. LEFT(XL, YL, ZL, XR, YR, ZR, X2, Y2, Z2)))) THEN
!
...
25                          IER = 5
                            WRITE (*, 100) NIT, IERR
100                         FORMAT(//5X, '*** Error in OPTIM (called from ', 'DELNOD):  NIT = ', I4, ', IER = ', I1, ' ***'/)
                            RETURN
!
